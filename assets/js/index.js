...
function searchPokemon(nombrePokemon){
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#stats").html(`
                <p class="pokeStats">Name: <strong>${data.name}</strong></p>
                ${data.types[1] == null ? `<p class="pokeStats">Type: <strong>${data.types[0].type.name}</strong></p>` : `<p class="pokeStats">Type (Principal): <strong>${data.types[0].type.name}</strong></p>`}
                ${data.types[1] == null ? " " : `<p class="pokeStats">Type (Secondary): <strong>${data.types[1].type.name}</strong></p>`}
                <p class="pokeStats">Weight: <strong>${data.weight}</strong> lbs.</p>
                <p class="pokeStats">Height: <strong>${data.height}</strong></p>
                ${data.abilities[1]==null ? `<p>Ability: <strong>${data.abilities[0].ability.name}</strong></p>` : `<p class="pokeStats">Ability(1): <strong>${data.abilities[0].ability.name}</strong></p>`} 
